// app/api/internal-schedules/[id]/route.ts
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const schedule = await updateInternalSchedule(id, {
      ...(date !== undefined ? { date } : {}),
    ...
    console.error("내부 일정 수정 실패:", error);

    return NextResponse.json(
      {
        message: "내부 일정 수정 중 오류가 발생했습니다.",
      },
      {
        status: 500,
      },
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const deleted = await deleteInternalSchedule(id);
    ...
    return NextResponse.json({
      message: "내부 일정을 삭제했습니다.",
    });
  } catch (error) {
    console.error("내부 일정 삭제 실패:", error);

    return NextResponse.json(
      {
        message: "내부 일정 삭제 중 오류가 발생했습니다.",
      },
      {
        status: 500,
      },
